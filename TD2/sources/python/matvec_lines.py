...
u_local = np.array(u)                              #Sous bloc de u pour avoir les bonnes dimensions 

v_local = np.array(A_local.dot(u_local))           #Calcul de chaque sous bloc
v= (np.array(comm.allgather(v_local))).ravel()     #On concatène tout et on envoie à tous les processus. Ravel sert a faire de v un array unidimensionnel
